...
import { faCircleCheck, faPause, faTrash, faPersonRunning } from '@fortawesome/free-solid-svg-icons'
...
export default function SessioniList({ sessioni }: { sessioni: Sessione[] }) {
  ...
  const eliminaSessione = async (id: string) => {
    setEliminando(id)
    // Prima elimina i log_serie collegati, poi la sessione
    ...
    <div>
      {lista.map((s, i) => (
        <div key={s.id} className="flex items-center gap-4 px-6 py-4"
          style={{ borderBottom: i < lista.length - 1 ? '1px solid oklch(1 0 0 / 4%)' : 'none' }}>
          ...
          <div className="w-10 h-10 rounded-xl flex items-center justify-center text-lg flex-shrink-0"
            ...
            {s.completata
              ...
              : <FontAwesomeIcon icon={faPause} style={{ color: 'oklch(0.45 0 0)' }} />}
          </div>

...
export default function RingCard({
  heroTitle, eyebrow, volumeNum, durata, serie, esercizi, highlights, coachName, dateExtended,
}: ShareCardInputProps) {
  const accent = 'oklch(0.78 0.20 50)'
  const accentRgb = 'rgba(235,140,60,1)'
  ...
  return (
    <div style={{
      ...
      <div style={{ textAlign: 'center', marginBottom: 16 }}>
        ...
        <h1 style={{ fontFamily: "'Syne', sans-serif", fontWeight: 900, fontSize: 44, letterSpacing: '-0.03em', lineHeight: '0.9', margin: 0 }}>{heroTitle}</h1>
        <div style={{ fontSize: 10, opacity: 0.65, fontWeight: 500, marginTop: 4, letterSpacing: '0.05em' }}>{dateExtended}</div>
      </div>

      {/* Ring */}
      <div style={{ display: 'flex', justifyContent: 'center', margin: '4px 0 8px', position: 'relative' }}>
        <svg width="200" height="200" viewBox="0 0 200 200" style={{ transform: 'rotate(-90deg)' }}>
          <circle cx="100" cy="100" r="85" fill="none" stroke="rgba(255,255,255,0.12)" strokeWidth="14" />
          <defs>
            <linearGradient id={gradId} x1="0" y1="0" x2="1" y2="1">
              <stop offset="0%" stopColor="oklch(0.82 0.17 85)" />
              <stop offset="100%" stopColor={accentRgb} />
            </linearGradient>
          </defs>
        ...
        <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{ fontFamily: "'Syne', sans-serif", fontWeight: 900, fontSize: 40, letterSpacing: '-0.03em', lineHeight: '0.9' }}>
            {volumeNum}<span style={{ fontSize: 18, opacity: 0.75 }}>kg</span>
          </div>
          <div style={{ fontSize: 8.5, letterSpacing: '0.22em', fontWeight: 700, opacity: 0.7, textTransform: 'uppercase', marginTop: 2 }}>Volume</div>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 14 }}>
        {[{ l: 'Serie', v: String(serie) }, { l: 'Durata', v: durata }].map(s => (
          <div key={s.l} style={{ textAlign: 'center', padding: 8, border: '1px solid rgba(255,255,255,0.18)', borderRadius: 10 }}>
            <div style={{ fontSize: 8, letterSpacing: '0.2em', fontWeight: 700, opacity: 0.6, textTransform: 'uppercase', marginBottom: 3 }}>{s.l}</div>
            <div style={{ fontFamily: "'Syne', sans-serif", fontWeight: 800, fontSize: 18, letterSpacing: '-0.01em' }}>{s.v}</div>
          </div>
        ))}
      </div>
      ...
      {/* Footer */}
      <div style={{ position: 'absolute', bottom: 40, left: 28, right: 28, display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingTop: 10, borderTop: '1px solid rgba(255,255,255,0.15)', fontSize: 9.5, opacity: 0.75 }}>
        <span>

  ...
  const fetchAll = async () => {
    ...
    const [setsRes, schedRes, clientiRes] = await Promise.all([
      ...
      supabase.from('progress_check_schedulazioni')
        .select(`*, profiles!progress_check_schedulazioni_cliente_id_fkey(full_name, telefono), progress_check_set(titolo), progress_check_risposte(id, inviato_at, visto_coach)`)
        .eq('coach_id', user.id)
